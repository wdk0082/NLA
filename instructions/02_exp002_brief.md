...
2. **Data.** Start with clean in-distribution contexts (single-domain FineFineWeb
   ...
   sentence start, not the raw page start), only the end position (the extraction token) is
   random, ≥ 50 tokens after that start, and it must be a whole word (no sentence-end cuts). Constructed contexts
...
4. **Three whole-explanation edit kinds, separate from per-claim contradictions:**
   ...
   structure (antonyms or unrelated words); final token replaced by "cat" everywhere it
   is mentioned. Pair them with a paraphrase of matched lexical change. One version of
   the final-token edit only; plausibility levels are parked.
