...
- Construction fix after review (second run of the cell): in 71 explanations the AV names the
  final token by the context's last few words (`Final token "What does"`), so the mechanical
  word swap gave `"What cat"` and, for unrelated_token, `"What from"`. The cat and
  unrelated_token variants now collapse a quoted label of ≤ 4 words ending in the token to the
  token alone (`nla.editor.collapse_token_phrase`; long quoted excerpts keep the word swap);
  unrelated_token is built for 255 of 256 activations.
...
### Conclusion
Cell `exp_002_r3` (n = 256 activations, the round-2 contexts / activations / explanations / resamples; var_nrm 0.441; hand edits: negations per sentence median 2.25, vocab content words changed median 0.92, paraphrase content words kept median 0.40):

| kind | n | FVE | dist to R(z) median | dist > resample floor | ΔL_h median | ΔL_o median (nats) | top-1 | S_x(z′) mean / median | NLI z→z′ entail / contra |
|---|---|---|---|---|---|---|---|---|---|
| orig | 256 | 0.776 | 0 | — | — | — | 0.90 | 0.56 / 0.71 | — |
| resample | 512 | 0.775 | 0.057 | 0.48 | — | — | 0.89 | — | — |
| paraphrase | 256 | 0.765 | 0.012 | 0.03 | 0.007 | 0.000 | 0.91 | 0.54 / 0.69 | 1.00 / 0.00 (bwd 1.00 / 0.00) |
| translate (fr) | 256 | 0.762 | 0.013 | 0.03 | 0.011 | 0.000 | 0.90 | 0.44 / 0.52 | 1.00 / 0.00 (bwd 1.00 / 0.00) |
| shuffle | 256 | 0.754 | 0.020 | 0.13 | 0.013 | 0.000 | 0.89 | 0.63 / 0.84 | 0.93 / 0.05 (bwd 0.92 / 0.06) |
| polarity flip (phrase level) | 256 | 0.767 | 0.007 | 0.004 | 0.006 | 0.000 | 0.89 | 0.80 / 0.94 | 0.00 / 1.00 |
| vocabulary swap (unrestricted) | 256 | 0.533 | 0.184 | 0.96 | 0.173 | 0.005 | 0.89 | 0.37 / 0.55 | 0.12 / 0.88 |
| final token → "cat" | 256 | 0.082 | 0.754 | 1.00 | 0.702 | 4.02 | 0.14 | 0.53 / 0.70 | 0.09 / 0.89 |
| unrelated_token | 255 | −0.623 | 1.468 | 1.00 | 1.369 | 0.153 | 0.72 | 0.44 / 0.47 | 0.65 / 0.15 |
| unrelated | 256 | −0.887 | 1.727 | 1.00 | 1.636 | 6.02 | 0.08 | 0.44 / 0.51 | 0.63 / 0.16 |
| ref identity F(h) | 256 | — | — | — | — | 0.000 | 1.00 | — | — |
| ref mean activation | 256 | 0 | — | — | — | 1.94 | 0.34 | — | — |

Matched pairs (per activation, Wilcoxon): polarity vs vocab dist −0.175 (p < 10⁻⁴; polarity larger in 0.4 %), ΔL_h −0.163 (p < 10⁻⁴), ΔL_o −0.005 (p < 10⁻⁴; polarity larger in 33 %); polarity vs paraphrase dist −0.004 (p < 10⁻⁴; polarity larger in 25 %), ΔL_h −0.002 (p = 0.15), ΔL_o 0.000 (p = 0.27); vocab vs paraphrase dist +0.171 (vocab larger in 100 %), ΔL_o +0.004 (70 %); unrelated vs unrelated_token dist +0.257 (unrelated larger in 97%), ΔL_h +0.236 (96%), ΔL_o +5.40 nats (99%). unrelated_token recovers a median 0.15 of the unrelated distance, 0.14 of its ΔL_h and 0.97 of its ΔL_o (ΔL_o < 1 nat for 89 % of unrelated_token vs 6 % of unrelated). Vocab: ΔL_h > 0.1 in 77 %, ΔL_o > 0.1 nats in 7 %. "cat": ΔL_h median 0.70 (p10 0.37, p90 1.03; > 0.1 in 98 %), ΔL_o median 4.02 nats (p10 0.81, p90 7.8; > 1 nat in 89 %); round 2's texts, with the multi-word token labels, gave 0.71 / 3.96. S_x(z′) − S_x(z) paired medians: polarity +0.14 (lower than z in 10 % of activations), vocab −0.06 (lower in 65 %), paraphrase −0.00, cat −0.01, unrelated_token −0.12. Alignment: AUC of dist separating H = 0 from H = 1 0.85 (round 2: 0.65); equal-error rate 0.19 at τ = 0.028; at the resample-median τ = 0.055 ε_steg 0.07, ε_alias 0.21 (judged equivalent: polarity 100 %, vocab 4 %, cat / unrelated / unrelated_token 0 %; judged different: paraphrase 3 %, translate 3 %, shuffle 14 %).

- Timing: edit 1 s, reconstruct 2815 texts 67 s (218 s with a cold page cache), output 3327 patches 114–177 s, NLI 10–15 s, analyze 3 s (3–6 min of GPU per run, two runs: the second after the token-label fix; the 139 GB of weights re-download in 2 min). Hand edits: 11 `hand-editor` agents (xhigh) of 16–24 items, 8–25 min each; a session limit killed 6 mid-way (their incremental writes survived) and 2 small agents finished the 15 missing items; the checker rewrite (multi-line quotes) cost one round of mechanical restoration + hand fixes (see Setup).
- H2 holds, and no longer inside the noise: a vocabulary swap that changes 92 % of the content words moves R(z) by 0.184 (3.3× the resample floor, above it for 96 % of activations, FVE 0.776 → 0.533), whereas negating every phrase moves it by 0.007 — less than a paraphrase (0.012; p < 10⁻⁴) and inside the floor for 99.6 % of activations. Neither changes the patched output (ΔL_o medians 0.000 and 0.005 nats).
- H4 splits by target: keeping only the final token in an otherwise unrelated explanation recovers 97 % of the unrelated output loss (KL 6.0 → 0.19 nats, top-1 agreement 0.08 → 0.72) but only 15 % of the unrelated activation distance (1.73 → 1.47) and 14 % of its ΔL_h; the token decides the output, the rest of the text decides most of R(z).
- H5 fails on the input side: the NLI judge rates the phrase-level flip as *more* entailed by the input than the original (S_x mean 0.80 vs 0.56, higher for 90 % of activations) while rating it a contradiction of the original explanation (NLI z→z′ 1.00), so S_x at the whole-explanation level does not detect negation on this genre; the AR treats the flip as equivalent (dist 0.007) as predicted. The vocabulary swap is the only edit S_x moves against (0.37, lower for 65 %).
- Label validity: paraphrase / translation entail 1.00 both ways, shuffle 0.93, polarity contradiction 1.00, vocab 0.88, cat 0.89; unrelated and unrelated_token are judged entailed 0.63–0.65 (genre-level overlap of the AV's explanations).
- Results in `experiments/results/exp_002_r3/` (summary.json, whole_effects.csv, alignment.csv, variants / recon_index / output / nli_variants / nli_x_whole CSVs, PNG plots, config); hand edits in `configs/hand_edits_exp002_r3.jsonl` (+ parts); walkthrough `experiments/results/exp_002_r3_walkthrough.html` (three pages: setup, the lifecycles of five activations drawn uniformly at random with seed 0 — idx 68, 78, 129, 161, 214 — and the population; the vocabulary swap is left out of the page).
