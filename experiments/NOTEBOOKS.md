...
| final token → "cat" | 256 | 0.080 | 0.754 | 0.98 | 0.708 | 3.96 | 0.14 | 0.54 / 0.70 | 0.11 / 0.88 |
| unrelated_token | 253 | −0.624 | 1.468 | 1.00 | 1.369 | 0.151 | 0.72 | 0.43 / 0.51 | 0.64 / 0.15 |
| unrelated | 256 | −0.887 | 1.727 | 1.00 | 1.636 | 6.02 | 0.08 | 0.44 / 0.51 | 0.63 / 0.16 |
| ref identity F(h) | 256 | — | — | — | — | 0.000 | 1.00 | — | — |
| ref mean activation | 256 | 0 | — | — | — | 1.94 | 0.34 | — | — |

Matched pairs (per activation, Wilcoxon): polarity vs vocab dist −0.175 (p < 10⁻⁴; polarity larger in 0.4 %), ΔL_h −0.163 (p < 10⁻⁴), ΔL_o −0.005 (p < 10⁻⁴; polarity larger in 33 %); polarity vs paraphrase dist −0.004 (p < 10⁻⁴; polarity larger in 25 %), ΔL_h −0.002 (p = 0.15), ΔL_o 0.000 (p = 0.27); vocab vs paraphrase dist +0.171 (vocab larger in 100 %), ΔL_o +0.004 (70 %); unrelated vs unrelated_token dist +0.257 (unrelated larger in 98 %), ΔL_h +0.237 (97 %), ΔL_o +5.41 nats (99 %). unrelated_token recovers a median 0.15 of the unrelated distance, 0.14 of its ΔL_h and 0.97 of its ΔL_o (ΔL_o < 1 nat for 89 % of unrelated_token vs 6 % of unrelated). Vocab: ΔL_h > 0.1 in 77 %, ΔL_o > 0.1 nats in 7 %. "cat": ΔL_h median 0.71 (p10 0.37, p90 1.03; > 0.1 in 98 %), ΔL_o median 3.96 nats (p10 0.85, p90 7.7; > 1 nat in 89 %), unchanged from round 2 (same texts). S_x(z′) − S_x(z) paired medians: polarity +0.14 (lower than z in 10 % of activations), vocab −0.06 (lower in 65 %), paraphrase −0.00, cat −0.01. Alignment: AUC of dist separating H = 0 from H = 1 0.85 (round 2: 0.65); equal-error rate 0.19 at τ = 0.028; at the resample-median τ = 0.055 ε_steg 0.07, ε_alias 0.21 (judged equivalent: polarity 100 %, vocab 4 %, cat / unrelated / unrelated_token 0 %; judged different: paraphrase 3 %, translate 3 %, shuffle 14 %).

- Timing: edit 1 s, reconstruct 2813 texts 67 s, output 3325 patches 114 s, NLI 2069 × 2 pairs + 2325 (x, z′) pairs 10 s, analyze 3 s (3 min of GPU; loads from the page cache after a 139 GB re-download, 2 min). Hand edits: 11 `hand-editor` agents (xhigh) of 16–24 items, 8–25 min each; a session limit killed 6 mid-way (their incremental writes survived) and 2 small agents finished the 15 missing items; the checker rewrite (multi-line quotes) cost one round of mechanical restoration + hand fixes (see Setup).
...
- Label validity: paraphrase / translation entail 1.00 both ways, shuffle 0.93, polarity contradiction 1.00, vocab 0.88, cat 0.88; unrelated and unrelated_token are judged entailed 0.63 (genre-level overlap of the AV's explanations).
